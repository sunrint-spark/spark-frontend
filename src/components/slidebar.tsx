...
const Slidebar: React.FC = () => {
    ...
    useEffect(()=>{
        async function getFlowList(){
            const response = await Api.getFlow() as Record<string, Record<string, string>[]>
    ...
    const ListPrint = flowList.map((item: Record<string, string>)=>{//나중에 리스트 구현할 예정
        return(
            <div className="slidebar-end-content-list" key={item.id} style={{
                cursor: 'pointer'
            }} onClick={
                async () => {
                    window.location.href = `/brainstorm/${item.id}`
                }
            }>
